...
class Neck(KM.Model):
    ...
    def call(self, C2, C3, C4):
        ...
        # TODO: add assert to varify feature map sizes match what's in config
        P4 = self.fpn_c5p5(C4)
        P3 = KL.Add(name="fpn_p4add")([ KL.UpSampling2D(size=(2, 2), name="fpn_p5upsampled")(P4),
                                        self.fpn_c4p4(C3)])
        P2 = KL.Add(name="fpn_p3add")([ KL.UpSampling2D(size=(2, 2), name="fpn_p4upsampled")(P3),
                                        self.fpn_c3p3(C2)])
        # Attach 3x3 conv to all P layers to get the final feature maps.
        P2 = self.fpn_p3(P2)
        P3 = self.fpn_p4(P3)
        ...
        P5 = KL.MaxPooling2D(pool_size=(1, 1), strides=2, name="fpn_p6")(P4)
