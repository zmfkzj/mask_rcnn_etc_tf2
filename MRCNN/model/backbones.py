...
def build_keras_backbone(model_name:str):
    ...
    output_layers = sorted([s for s in set([o.ref() for o in output_layers])], key=lambda x: [l.ref() for l in output_layers].index(x))
    output_layers = [l.deref() for l in output_layers]
    outputs = {}
    for i, o in enumerate(output_layers[-4:]):
        outputs[str(i+2)] = o
...
backbones = {
    ...
    'InceptionV3': (lambda: tf.keras.applications.InceptionV3(include_top=False, input_tensor=tf.keras.Input((None,None,3), dtype=tf.float16)) , build_keras_backbone),
