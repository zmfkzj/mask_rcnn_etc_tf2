...
class DataLoader(frcnn_data_loader.DataLoader):
    ...
    def __post_init__(self):
        if self.phase==1:
            self.dataset.__set_dataloader_class_list(self.config.NOVEL_CLASSES)
            self.dataset.__count_each_class_objects(self.config.NOVEL_CLASSES)
            self.config.set_phase(1)
        else:
            self.config.set_phase(2)
            if self.mode != Mode.PREDICT:
                self.dataset.__set_dataloader_class_list([])
                self.dataset.__count_each_class_objects([])
