...
class ProposalLayer(KL.Layer):
    ...
    def call(self, inputs):
        ...
        deltas = inputs[1]
        deltas = deltas * tf.cast(tf.reshape(self.config.RPN_BBOX_STD_DEV, [1, 1, 4]),tf.float16)
        # Anchors
        anchors = inputs[2]
        ...
        # pre_nms_anchors = tf.cast(tf.vectorized_map(lambda x: denorm_boxes_graph(x,list(self.config.IMAGE_SHAPE)[:2]),pre_nms_anchors), tf.float16)

        # Apply deltas to anchors to get refined anchors.
        # [batch, N, (y1, x1, y2, x2)]
        boxes = tf.map_fn(lambda x:apply_box_deltas_graph(*x),
                          [pre_nms_anchors,deltas], 
                          fn_output_signature=tf.TensorSpec(shape=(self.config.PRE_NMS_LIMIT, 4), dtype=tf.float16))

        # Clip to image boundaries. Since we're in normalized coordinates,
        # window = tf.stack([0., 0., *list(self.config.IMAGE_SHAPE-1)[:2]])
        window = tf.cast(tf.stack([0., 0., 1., 1.]), tf.float16)
        ...
        proposals = tf.map_fn(nms,(boxes, scores),
                              fn_output_signature=tf.TensorSpec(shape=[None,4], dtype=tf.float16))
