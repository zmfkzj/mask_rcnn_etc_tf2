...
for b in backbones:
    ...
    if resume:
        model.load_weights('save_EfficientNetV2L_2023-06-16T03:11:01.932688/chpt/nas_fpn/train_loss')
        model.compile()
        resume = False


    if config.FPN == 'NASFPN':
        ###########################
        # NAS FPN train
        ###########################

        callbacks = [tf.keras.callbacks.ModelCheckpoint(f'save_{b}_{now}/chpt/nas_fpn/best',monitor='val_mAP85',save_best_only=True,mode='max', save_weights_only=True),
                    tf.keras.callbacks.ModelCheckpoint(f'save_{b}_{now}/chpt/nas_fpn/train_loss',monitor='mean_loss',save_best_only=True,mode='min', save_weights_only=True),
                    tf.keras.callbacks.EarlyStopping('val_mAP85',patience=3,verbose=1, mode='max',restore_best_weights=True),
                    tf.keras.callbacks.TensorBoard(log_dir=f'save_{b}_{now}/logs/nas_fpn')]

        model.set_trainable(TrainLayers.ALL)
        model.neck.set_nas_train_mode(True)
        with config.STRATEGY.scope():
        ...
        model.neck.set_nas_train_mode(False)
    ...
    # callbacks = [tf.keras.callbacks.ModelCheckpoint(f'save_{b}_{now}/chpt/head/best',monitor='val_mAP85',save_best_only=True,mode='max'),
    #             tf.keras.callbacks.ModelCheckpoint(f'save_{b}_{now}/chpt/head/train_loss',monitor='mean_loss',save_best_only=True,mode='min'),
    ...
    callbacks = [tf.keras.callbacks.ModelCheckpoint(f'save_{b}_{now}/chpt/fingtune/best',monitor='val_mAP85',save_best_only=True,mode='max', save_weights_only=True),
                tf.keras.callbacks.ModelCheckpoint(f'save_{b}_{now}/chpt/fingtune/train_loss',monitor='mean_loss',save_best_only=True,mode='min', save_weights_only=True),
                tf.keras.callbacks.TensorBoard(log_dir=f'save_{b}_{now}/logs/fingtune'),
                tf.keras.callbacks.ReduceLROnPlateau(monitor='mean_loss', mode='min',patience=3),
                tf.keras.callbacks.EarlyStopping('val_mAP85',patience=3,verbose=1, mode='max',restore_best_weights=True)]
    ...
    # evaluate
    ###########################
    result = model.evaluate(val_loader,steps=300)
