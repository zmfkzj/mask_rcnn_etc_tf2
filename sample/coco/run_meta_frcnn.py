...
config = Config(GPUS=0,
                ...
                NOVEL_CLASSES=(81,82,83,84,85,86,87,88),
                SHOTS=0,
                )
...
train_dataset = Dataset('/home/tmdocker/host/dataset/5_coco_merge/annotations/instances_train.json', 
                    '/home/tmdocker/host/dataset/5_coco_merge/images')
val_dataset = Dataset('/home/tmdocker/host/dataset/5_coco_merge/annotations/instances_test.json', 
                    '/home/tmdocker/host/dataset/5_coco_merge/images')
...
lr_schedule = CustomScheduler(config.LEARNING_RATE, 20*config.STEPS_PER_EPOCH,0.6,1, staircase=True)
with config.STRATEGY.scope():
    ...
    model.load_weights('save_2023-03-21T06:37:18.023996/chpt/phase2_all_best.h5', by_name=True, skip_mismatch=True)
...
lr_schedule = CustomScheduler(config.LEARNING_RATE/100, 30*config.STEPS_PER_EPOCH,0.5,1, staircase=True)
with config.STRATEGY.scope():
    optimizer = keras.optimizers.Adam(learning_rate=lr_schedule, clipnorm=config.GRADIENT_CLIP_NORM)
